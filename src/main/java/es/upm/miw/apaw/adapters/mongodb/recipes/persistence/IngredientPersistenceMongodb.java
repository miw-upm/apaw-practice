...
public class IngredientPersistenceMongodb implements IngredientPersistence {
    ...
    public Ingredient update(Ingredient ingredient) {
        IngredientEntity ingredientEntity = this.ingredientRepository
                .readByLabel(ingredient.getLabel())
                .orElseThrow(() -> new NotFoundException("Ingredient with label: " + ingredient.getLabel() + " not found"));
        ingredientEntity.setMarketPrice(ingredient.getMarketPrice());

        return this.ingredientRepository.save(ingredientEntity).toIngredient();
    }
}
